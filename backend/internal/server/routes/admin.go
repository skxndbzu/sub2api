...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/turn-state/status", h.Admin.Account.GetTurnStateStatus)
		accounts.POST("/:id/turn-state/probe", h.Admin.Account.ProbeTurnState)
		accounts.DELETE("/:id/turn-state/cache", h.Admin.Account.ClearTurnState)

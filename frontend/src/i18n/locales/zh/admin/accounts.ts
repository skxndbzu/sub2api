export default {
    accounts: {
      turnState: {
        title: 'OpenAI 会话状态',
        description: '按账号、最终模型和服务档位复用状态。自动解析 token 签发时间并计算本地到期时间；长度特征不代表官方质量保证。',
        enabled: '启用会话状态探测与自动注入',
        proxies: '探测出口代理（多选）',
        noProxies: '请先在代理管理中添加可用节点。',
        egressHint: '不同代理地址可能共用公网出口；后台验证并去重，不生成 IPv6。请使用固定或粘性出口。',
        models: '最终上游模型与有效服务档位',
        modelsHint: '选择实际发往上游的模型 ID；每个档位独立探测和缓存。',
        omittedTier: '未指定档位',
        length: '目标状态长度（字节）',
        ttl: '自签发起的最大有效时长（秒）',
        expiryHint: '本地到期时间 = token 签发时间 + 最大有效时长，读取缓存不会续期。Fernet 时间戳不是过期时间；上游实际有效期和密钥轮换时间无法从该字段确定。解析失败或已过期的状态不会缓存。',
        refreshBefore: '提前刷新阈值（秒）',
        attempts: '单轮最多候选节点',
        timeout: '单节点超时（秒）',
        rawLog: '打印上游响应状态原值',
        rawLogHint: '开启后，日志记录实际响应头 x-codex-turn-state 的完整值；此开关独立于自动注入。',
        savedConfig: '操作使用已保存的账号配置。探测可能产生上游用量；关闭响应流不代表零消耗。',
        saveFirst: '保存账号后，可在编辑界面立即探测、清空缓存并查看状态。',
        probeNow: '立即探测', clear: '清空缓存',
        modelTier: '模型 / 档位', status: '状态', details: '诊断',
        disabled: '已关闭', refreshing: '刷新中', characters: '字节',
        issuedAt: 'Token 签发时间',
        probedAt: '探测时间', expiresAt: '本地到期时间', retryAt: '下次可探测',
        queued: '探测已排队；相同账号和模型的任务会合并或串行执行。',
        cleared: '缓存已清空。在途旧探测不能回写；后续业务缺失可触发新探测。',
        statusUnavailable: '状态暂不可读取，请稍后重试。',
        operationFailed: '操作未完成，请检查已保存的配置、账号状态及后台服务。',
        bulkHint: '仅修改勾选的项目；未勾选项保留各账号原有配置。',
        changeEnabled: '修改自动注入开关', changeLog: '修改原值日志开关',
        states: { unprobed: '未探测', probing: '探测中', available: '可用', expired: '已过期', failed: '探测失败' }
      },
      title: '账号管理',

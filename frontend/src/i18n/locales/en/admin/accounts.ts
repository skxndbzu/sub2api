export default {
    accounts: {
      turnState: {
        title: 'OpenAI turn state',
        description: 'Reuse states matching a length feature per account, final model and service tier. 292 characters and one hour are local policies, not official quality or validity guarantees.',
        enabled: 'Enable state probing and automatic injection',
        proxies: 'Probe egress proxies (multiple)',
        noProxies: 'Add available nodes in proxy management first.',
        egressHint: 'Different proxy addresses may share a public IP. Egresses are checked and deduplicated; IPv6 addresses are never generated. Use fixed or sticky egresses.',
        models: 'Final upstream models and effective service tiers',
        modelsHint: 'Select actual upstream model IDs. Each tier has its own probe and cache.',
        omittedTier: 'Tier omitted',
        length: 'Target state length (bytes)',
        ttl: 'Maximum age since issuance (seconds)',
        expiryHint: 'Local expiry = token issuance time + maximum age; cache reads do not extend it. The Fernet timestamp is not an expiry claim and does not reveal upstream lifetime or key rotation. Invalid or expired states are not cached.',
        issuedAt: 'Token issued',
        refreshBefore: 'Refresh before expiry (seconds)',
        attempts: 'Maximum candidate nodes per round',
        timeout: 'Timeout per node (seconds)',
        rawLog: 'Log the upstream response state value',
        rawLogHint: 'Logs the complete x-codex-turn-state response header when enabled. This setting is independent of automatic injection.',
        savedConfig: 'Actions use the saved account configuration. Probes may incur upstream usage; closing a response stream does not imply zero cost.',
        saveFirst: 'Save the account, then edit it to probe, clear the cache and view status.',
        probeNow: 'Probe now', clear: 'Clear cache',
        modelTier: 'Model / tier', status: 'Status', details: 'Diagnostics',
        disabled: 'Disabled', refreshing: 'Refreshing', characters: 'bytes',
        probedAt: 'Probed', expiresAt: 'Local expiry', retryAt: 'Next probe allowed',
        queued: 'Probe queued. Tasks for the same account and model are merged or run serially.',
        cleared: 'Cache cleared. Old probes cannot restore it; a later business cache miss can trigger a new probe.',
        statusUnavailable: 'Status is temporarily unavailable. Please try again.',
        operationFailed: 'Action could not complete. Check the saved configuration, account status and backend service.',
        bulkHint: 'Only checked fields are changed. Other settings are preserved for each account.',
        changeEnabled: 'Change automatic injection setting', changeLog: 'Change raw value logging',
        states: { unprobed: 'Not probed', probing: 'Probing', available: 'Available', expired: 'Expired', failed: 'Probe failed' }
      },
